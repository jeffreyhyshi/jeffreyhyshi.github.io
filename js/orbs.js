...
function initializeOrbCallback(item, svg, letter) {
	...
	numLoaded += 1;
	if (numLoaded == MAGIC) {
		layout([['j', 'e1', 'f1', 'f2', 'r', 'e2', 'y'],
					 ['s', 'h', 'i']], 7);
	}
}

function layout(arr, numBalls) {
	//Figure out how to center 'em
	var ballsWidth = numBalls * 2 * ORB_RADIUS;

	var topCenter = new Point(($(window).width() - ballsWidth) / 2.5, TOP_DIST);
	var leftOffset = Math.max(100, topCenter.x - 6 * ORB_RADIUS)
	...
	for (var row = 0; row < arr.length; row++) {
		for (var col = 0; col < arr[row].length; col++) {
			var key = arr[row][col];
			if (key in allOrbs) {
				lastPoint = new Point(leftOffset + ORB_DIAM * col, 
	...
	$('.content-wrapper').offset({left: leftOffset, top: lastPoint.y + 3 * ORB_RADIUS});

	//Move that first letter out and have it start with some velocity
	initialOrbPosition =
	new Point(allOrbs['j'].orbShape.position.x - Math.randRange(ORB_RADIUS, ORB_RADIUS * 2),
		allOrbs['j'].orbShape.position.y - Math.randRange(ORB_RADIUS, ORB_RADIUS * 2));

	//Fade out cover div
	$('.cover').fadeOut();
	initialOrbVelocity = new Point(Math.randRange(1,3), Math.randRange(1,3));
	allOrbs['j'].orbShape.position = initialOrbPosition;
	allOrbs['j'].velocity = initialOrbVelocity;
}
